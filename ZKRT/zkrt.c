...
uint8_t now_uav_type = SERSOR_8_NOWTYPE;              //区别4探头和8探头
uint16_t now_uav_num = 0;

extern uint8_t addrdata[NO_OF_SENSOR];
extern uint16_t sen_val_buff[NO_OF_SENSOR];
...
//气体num值定义，在应用协议上的定义
#define NONE_S_TYPE_PTC 0
#define CO_S_TYPE_PTC   1
#define H2S_S_TYPE_PTC  2
#define NH3_S_TYPE_PTC  3
#define CO2_S_TYPE_PTC  4
#define CL2_S_TYPE_PTC  5
#define SO2_S_TYPE_PTC  6
#define VOC_S_TYPE_PTC  7
#define CH4_S_TYPE_PTC  8

const uint8_t ptc_to_poison_type[NO_OF_SENSOR] = {CO, H2S, NH3, CO2, CL2, SO2, TVOC, CH4};

////对于特定的毒气值进行封包，返回封包结果
//void zkrt_read_posion_ack(uint8_t sersor_num_type)  //SERSOR_8_NOWTYPE
//{
//	uint8_t i;
...
//	packet.data[0]=0xfe;
//	packet.data[1] = 1;
//	packet.data[2] = (uint8_t)(sen_val_buff[CO_ADD-1]&0xff);
//	packet.data[3] = (uint8_t)(sen_val_buff[CO_ADD-1]>>8);
//	//save H2S
//	packet.data[4]=0xfe;
//	packet.data[5] = 2;
//	packet.data[6] = (uint8_t)(sen_val_buff[H2S_ADD-1]&0xff);
//	packet.data[7] = (uint8_t)(sen_val_buff[H2S_ADD-1]>>8);
//	//save NH3
//	packet.data[8]=0xfe;
//	packet.data[9] = 3;
//	packet.data[10] = (uint8_t)(sen_val_buff[NH3_ADD-1]&0xff);
//	packet.data[11] = (uint8_t)(sen_val_buff[NH3_ADD-1]>>8);
//	//save CO2
//	packet.data[12]=0xfe;
//	packet.data[13] = 4;
//	packet.data[14] = (uint8_t)(sen_val_buff[CO2_ADD-1]&0xff);
//	packet.data[15] = (uint8_t)(sen_val_buff[CO2_ADD-1]>>8);
//	//save TVOC  //next message is different as above, because the compatibly of 4 sersors 
//	packet.data[16] = (uint8_t)(sen_val_buff[TVOC_ADD-1]&0xff);
//	packet.data[17] = (uint8_t)(sen_val_buff[TVOC_ADD-1]>>8);	
//	//save CL2
//	packet.data[18] = (uint8_t)(sen_val_buff[CL2_ADD-1]&0xff);
//	packet.data[19] = (uint8_t)(sen_val_buff[CL2_ADD-1]>>8);	
//	//save SO2
//	packet.data[20] = (uint8_t)(sen_val_buff[SO2_ADD-1]&0xff);
//	packet.data[21] = (uint8_t)(sen_val_buff[SO2_ADD-1]>>8);	
//	//save CH4
//	packet.data[22] = (uint8_t)(sen_val_buff[CH4_ADD-1]&0xff);
//	packet.data[23] = (uint8_t)(sen_val_buff[CH4_ADD-1]>>8);		
//	for(i=24;i<30;i++)
//	{
//		packet.data[i] = 0;
//	}
...
//对于特定的毒气值进行封包，返回封包结果
void zkrt_read_posion_ack(uint8_t sersor_num_type)  //SERSOR_4_NOWTYPE
{
	uint8_t i;
	zkrt_packet_t packet;
	...
	packet.UAVID[0] = sersor_num_type;
	packet.UAVID[1] = (uint8_t)(now_uav_num&0xff);
	packet.UAVID[2] = (uint8_t)(now_uav_num>>8);
	packet.UAVID[3] = DEVICE_TYPE_GAS;
	packet.UAVID[4] = DEFAULT_NUM;
	...
	//save CO
	packet.data[0]=0xfe;
	packet.data[1] = CO_S_TYPE_PTC;
	packet.data[2] = (uint8_t)(sen_val_buff[1-1]&0xff);
	packet.data[3] = (uint8_t)(sen_val_buff[1-1]>>8);
	//save H2S
	packet.data[4]=0xfe;
	packet.data[5] = H2S_S_TYPE_PTC;
	packet.data[6] = (uint8_t)(sen_val_buff[3-1]&0xff);
	packet.data[7] = (uint8_t)(sen_val_buff[3-1]>>8);
	//save CO2
	packet.data[8]=0xfe;
	packet.data[9] = CO2_S_TYPE_PTC;
	packet.data[10] = (uint8_t)(sen_val_buff[4-1]&0xff);
	packet.data[11] = (uint8_t)(sen_val_buff[4-1]>>8);
	//save CH4
	packet.data[12]=0xfe;
	packet.data[13] = CH4_S_TYPE_PTC;
	packet.data[14] = (uint8_t)(sen_val_buff[8-1]&0xff);
	packet.data[15] = (uint8_t)(sen_val_buff[8-1]>>8);
//	//save TVOC  //next message is different as above, because the compatibly of 4 sersors 
//	packet.data[16] = (uint8_t)(sen_val_buff[TVOC_ADD-1]&0xff);
//	packet.data[17] = (uint8_t)(sen_val_buff[TVOC_ADD-1]>>8);	
//	//save CL2
