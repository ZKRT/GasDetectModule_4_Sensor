...
uint8_t addrdata[NO_OF_SENSOR]={0};
uint32_t sensor_type=0;//32位标志位

uint8_t ControlType=Bit7;/*7->7Bit 6->6Bit 5->5Bit*/

//ADD BY YANLY
extern uint8_t addr_valid_flag[NO_OF_SENSOR];
extern uint8_t addr_sersor_type[NO_OF_SENSOR];
...
uint8_t ReadMeaVal (uint8_t address, uint16_t *meaval)
{
 //  static uint8_t temp=1;
   ...
   ControlType = Bit7;
	 delay_xms (50);            //两个发送帧之间间隔3.5ms以上，这里 5ms
   RS485_Receive_Data(meaVal_data[address-1], ControlType);
	 for(k=0;k<7;k++)
	 {
		 rec_data[k] = meaVal_data[address-1][k];
	 }
   check = GetCheck(rec_data, 5);
   if((check&0xff) == rec_data[5] && ((check>>8)&0XFF) ==rec_data[6])
   {
...
void CheckSensorType(void)
{
   ...
   uint8_t type=0;
   delay_xms(10);
   for(addr=1; addr<NO_OF_SENSOR+1; addr++)
   {
			type=0;
			if(ReadType(addr, &type)) 
			{
				addr_valid_flag[addr-1] = 1;
				addr_sersor_type[addr-1] = type;
			}
			delay_xms(10);
   }
}
...
/**
  * @brief read all sersor data
  * @param   
  * @retval 
  */
void ReadAllVal_V2(uint16_t *buff)
{
	memset(buff,0X00,NO_OF_SENSOR); 
	for(_i=1; _i<=NO_OF_SENSOR; _i++)
	{
		if(addr_valid_flag[_i-1] == 1)
		{
			ReadMeaVal(_i, &buff[_i-1]);
		}
		else
		{
			buff[_i-1] = 0;
...
/**
  * @brief 根据类型获取对应类型当前地址
  * @param   
  * @retval //0表示获取不到
  */
uint8_t get_addr_in_typearry(uint8_t type)
{
	int i;
	for(i=0; i<NO_OF_SENSOR; i++)
	{
		if(type==addr_sersor_type[i])
		{
			return i+1;
		}
	}
