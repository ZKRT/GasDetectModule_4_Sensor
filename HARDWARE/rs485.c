...
volatile uint8_t  receiveValue[7];
...
void RS485_Init(uint32_t bound)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8; 
   ...
   //USART1 初始化设置
	...
	//USART_InitStructure.USART_WordLength = USART_WordLength_8b;//字长为9位数据格式,八位会不正常 //zkrt_debug
...
{
	uint8_t i;
	memset(buf, 0x00, len);  //modify by yanly
	if(receiveFlag)
	{
		receiveFlag = 0;
